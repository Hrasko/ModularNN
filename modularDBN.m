...
for i=1:k
    dnnname = listing(sorted(i)).name;
    dnn = load(dnnname);
    dnn = dnn.dnn;
    dnnsave = strcat('dnn',num2str(i));
    save(dnnsave,'dnn');
    netsplit = strsplit(dnnname,'DNN');
    netname = strcat(netsplit(1),'NET.mat');
    net = load(netname{1});
    net = net.net;
    netsave = strcat('net',num2str(i));
    save(netsave,'net');
    paramssplit = strsplit(netsplit{1},'x');
    delay = str2double(paramssplit(2));
    delta = str2double(paramssplit(3));
    ...
    [modNN{i}.s, opt.in, opt.out] = sampleDBNFromDB(dnn,net,dbmod,delay,delta);
