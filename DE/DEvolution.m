...
function difEv = DEvolution(nPop,dim,varargin)
    ...
    for k=1:difEv.opt.maxIter
        ...
        if (difEv.opt.verbose)
            iteracao = k
            ind = difEv.bestInd.value
            err = difEv.bestInd.erro
        end % if
...
function pop = initPop (difEv)
    ...
    elseif (difEv.opt.intervalInit == -1)
        % inicializa todos iguais
        pop = ones(difEv.nPop,difEv.dim) * (round((1/difEv.dim)*100)/100); 
    else        
...
function opt = loadOpt (opt)   
    ...
    if( isfield(opt,'tipo') == 0 )
        opt.tipo = 0;
    end
    
    if( isfield(opt,'verbose') == 0 )
        opt.verbose = true;
    end
...
function difEv = mutation (difEv)
    ...
    for i=1:difEv.nPop
        ...
        for j=1:difEv.dim
            ...
            % rever isso
            if (difEv.opt.tipo == 0)
                if (difEv.popMut.values(i,j) < 0 || difEv.popMut.values(i,j) > 1)
                    difEv.popMut.values(i,j) = rand ();
                end
            end
        end % for                        
    end % for    
...
function difEv = selection (difEv)
    ...
    for i=1:difEv.nPop
        ...
        if (difEv.opt.tipo == 0)
            difEv.popC.erros(i) = Fitness (difEv.popC.values(i,:),difEv.opt.IN);
            difEv.popCross.erros(i) = Fitness (difEv.popCross.values(i,:),difEv.opt.IN);
        else
            difEv.popC.erros(i) = Fitness_raiz (difEv.popC.values(i,:));
            difEv.popCross.erros(i) = Fitness_raiz (difEv.popCross.values(i,:));
        end
